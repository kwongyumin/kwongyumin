<div align="center">
  <h1>도메인을 이해하고 확장성을 고려하는 백엔드 개발자</h1>
  <p>KG모빌리언스 · 헥토데이터에서 4년+ 핀테크 백엔드 개발 경험이 있습니다.</p>
</div>

<br>

## 👋 안녕하세요, 권규민입니다.

- ⚙️ 트랜잭션 경계, 락 전략, 동시성 이슈를 세밀하게 설계하는 것을 좋아합니다
- 🔍 운영 장애를 임시방편 없이 근본 원인까지 파고드는 것을 원칙으로 합니다
- 📝 코드 리뷰에 적극 참여하고 API 명세·장애 처리 절차를 문서화합니다
- 🏅 AWS Certified Solutions Architect – Associate

<br>

## 📈 GitHub Stats

<div align="center">

[![Anurag's github stats](https://github-readme-stats.vercel.app/api?username=kwongyumin&theme=merko)](https://github.com/anuraghazra/github-readme-stats)
[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=kwongyumin&layout=compact&theme=merko&hide=html,css)](https://github.com/anuraghazra/github-readme-stats)

</div>

<br>

## 🛠 Tech Stack

**Language & Backend**

<div>
  <img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=Java&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=Spring-Boot&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_WebFlux-6DB33F?style=for-the-badge&logo=Spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Batch-6DB33F?style=for-the-badge&logo=Spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/MyBatis-DC382D?style=for-the-badge&logoColor=white"/>
</div>

**Database & Cache**

<div>
  <img src="https://img.shields.io/badge/Oracle-F80000?style=for-the-badge&logo=Oracle&logoColor=white"/>
  <img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=Redis&logoColor=white"/>
</div>

**Infra & DevOps**

<div>
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=Linux&logoColor=black"/>
  <img src="https://img.shields.io/badge/Jenkins-D24939?style=for-the-badge&logo=Jenkins&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=Git&logoColor=white"/>
</div>

<br>

## ⚡ Experience Highlights

| 경험 영역 | 내용 |
|-----------|------|
| 동시성 · 정합성 | 분산 서버 환경에서 Lock 전략, 트랜잭션 전파 레벨 설계로 정합성 오류 해소 |
| 성능 최적화 | Covering Index, 비동기 분리, 쿼리 튜닝으로 API·쿼리 응답 속도 개선 |
| 대용량 배치 처리 | Spring Batch Chunk 기반으로 수십만 건 데이터 안정적 처리 |
| JVM 메모리 분석 | jstat · jmap으로 메모리 누수 원인 특정 및 해소 |
| 보안 대응 | mTLS 인증 오류 분석, 모의해킹 기반 보안 취약점 개선 주도 |
| ConnectionPool | Dead Lock 원인 분석 및 Pool 최적값 도출 |

<br>

## 🍀 Contact

<div>
  <a href="https://blog.naver.com/rbals7398">
    <img src="https://img.shields.io/badge/Tech%20Blog-03C75A?style=flat-square&logo=Naver&logoColor=white"/>
  </a>
  <a href="mailto:rbals7398@naver.com">
    <img src="https://img.shields.io/badge/rbals7398@naver.com-d14836?style=flat-square&logo=Naver&logoColor=white"/>
  </a>
</div>
